  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      stopLane(id) {
        ...
        if (!el) return;
        el.getAnimations().forEach((anim) => anim.playState === 'running' && anim.pause());
  ...
  useEffect(() => {
    ...
    (async () => {
      const settled = await Promise.allSettled(
        carsToAnimate.map(async (car) => {
          ...
          try {
            ...
            if (!isStartRes(resUnknown)) throw new Error('Invalid start-engine payload');
          ...
          const parentW = laneEl.parentElement!.getBoundingClientRect().width;
          const carW = laneEl.getBoundingClientRect().width;
          const free = Math.max(parentW - carW, 0);

          const { transform } = getComputedStyle(laneEl);
          const parts = transform.startsWith('matrix') ? transform.slice(7, -1).split(',') : [];
          ...
          try {
            ...
            if (!isDriveRes(driveUnknown) || !driveUnknown.success) throw new Error();
          } catch {
            anim.pause();
      ...
      const results = settled.map<Result>((r) =>
        r.status === 'fulfilled' && r.value ? r.value : { id: -1, time: Infinity },
      );
      ...
      onRaceEnd(`🏆 ${winCar.name} wins in ${(winner.time / 1000).toFixed(2)} s`);

      /* ---------- 7. Update / create winner ---------- */
      try {
        const resp = await fetch(`${import.meta.env.VITE_API}/winners/${winner.id}`);
        if (resp.ok) {
          const raw: unknown = await resp.json();
          if (typeof raw === 'object' && raw !== null && 'wins' in raw && 'time' in raw) {
        ...
        console.error('Failed to update/create winner', e instanceof Error ? e : String(e));
      }
    })();
